...
int main(int argc, char *argv[]) {
  ...
  aligned_vector<float> array_a1(N);
  aligned_vector<float> array_a2(N);
  aligned_vector<float> array_b1(N);
  aligned_vector<float> array_b2(N);
  aligned_vector<float> array_c_cpu(N);
  aligned_vector<float> array_c_fpga(N);

  srand(0);
  for (int i = 0; i < N; i++) {
    array_a1[i] = static_cast<float>(rand() % 100);
